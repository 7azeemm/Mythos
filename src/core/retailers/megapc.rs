use crate::core::retailers::utils::validate_url;
use crate::core::retailers::{Retailer, RetailerConfig};
use crate::core::sections::Section;
use crate::core::tracking::scrape_error::ProductParseError;
use crate::utils::scraper_ext::ElementRefExt;
use crate::utils::web_client::WebClientType;
use once_cell::sync::Lazy;
use scraper::{ElementRef, Selector};

static CONFIG: RetailerConfig = RetailerConfig {
    name: "MegaPC",
    web_client_type: WebClientType::Browser,
    nav_sel: Lazy::new(|| Selector::parse("button.rounded-md.bg-gray-200").unwrap()),
    product_sel: Lazy::new(|| Selector::parse("article.product-card").unwrap()),
    title_sel: Lazy::new(|| Selector::parse("a[href]").unwrap()),
    image_sel: Lazy::new(|| Selector::parse("div.card-img-container img").unwrap()),
    price_sel: Lazy::new(|| Selector::parse("span.inline-block").unwrap()),
    old_price_sel: Lazy::new(|| Selector::parse("del.text-sm").unwrap()),
    price_sel_2: None,
    status_sel: None,
    desc_sel: None,
    page_desc_sel: Some(Lazy::new(|| Selector::parse("div.productView-info").unwrap())),
    empty_page_sel: None,
    sections: &[
        (Section::PC, "https://megapc.tn/shop/ORDINATEURS/BAREBONE"),
        (Section::PC, "https://megapc.tn/shop/ORDINATEURS/PRO%20PC"),
        (Section::PC, "https://megapc.tn/shop/AI%20WORKSTATIONS/AI-READY%20POWERHOUSES"),
        (Section::GamingPC, "https://megapc.tn/shop/ORDINATEURS/PC%20GAMER"),
        (Section::GamingPC, "https://megapc.tn/shop/ORDINATEURS/LEGENDARY"),
        (Section::GamingPC, "https://megapc.tn/shop/search/PREBUILD"),
        (Section::GamingPC, "https://megapc.tn/shop/ORDINATEURS/FULL%20SETUP"),
        (Section::AllInOnePC, "https://megapc.tn/shop/ORDINATEURS/PC%20TOUT%20EN%20UN"),
        (Section::Laptop, "https://megapc.tn/shop/PC%20PORTABLE/PC%20PORTABLE%20PRO"),
        (Section::Laptop, "https://megapc.tn/shop/AI%20WORKSTATIONS/AI-READY%20LAPTOPS"),
        (Section::GamingLaptop, "https://megapc.tn/shop/PC%20PORTABLE/PC%20PORTABLE%20GAMER"),
        (Section::Monitor, "https://megapc.tn/shop/ECRANS/ECRANS%20GAMING"),
        (Section::Monitor, "https://megapc.tn/shop/ECRANS/ECRANS%20PRO"),
        (Section::CPU, "https://megapc.tn/shop/COMPOSANTS/PROCESSEUR"),
        (Section::GPU, "https://megapc.tn/shop/COMPOSANTS/CARTE%20GRAPHIQUE"),
        (Section::GPU, "https://megapc.tn/shop/ORDINATEURS/CUSTOM%20BUILD"),
        (Section::Memory, "https://megapc.tn/shop/COMPOSANTS/BARETTE%20M%C3%89MOIRE"),
        (Section::Storage, "https://megapc.tn/shop/STOCKAGE/DISQUE-SSD"),
        (Section::Storage, "https://megapc.tn/shop/STOCKAGE/DISQUE-NVME"),
        (Section::Storage, "https://megapc.tn/shop/STOCKAGE/DISQUE-HDD"),
        (Section::Motherboard, "https://megapc.tn/shop/COMPOSANTS/CARTE%20M%C3%88RE"),
        (Section::Cooler, "https://megapc.tn/shop/COMPOSANTS/REFROIDISSEMENT"),
        (Section::PowerSupply, "https://megapc.tn/shop/COMPOSANTS/ALIMENTATION"),
        (Section::Case, "https://megapc.tn/shop/COMPOSANTS/BOITIER"),
        (Section::Mouse, "https://megapc.tn/shop/ACCESSOIRES/SOURIS"),
        (Section::Keyboard, "https://megapc.tn/shop/ACCESSOIRES/CLAVIER"),
        (Section::MousePad, "https://megapc.tn/shop/ACCESSOIRES/TAPIS"),
        (Section::Headphones, "https://megapc.tn/shop/ACCESSOIRES/CASQUE"),
        (Section::Headphones, "https://megapc.tn/shop/SMARTPHONE%20ACCESSOIRES/EARBUDS"),
        (Section::GamingChair, "https://megapc.tn/shop/ACCESSOIRES/CHAISE%20GAMING"),
        (Section::AccessoriesCombo, "https://megapc.tn/shop/ACCESSOIRES/COMBO"),
        (Section::UpgradeKit, "https://megapc.tn/shop/COMPOSANTS/KIT%20UPGRADE%20PC"),
        (Section::Console, "https://megapc.tn/shop/CONSOLES/PLAYSTATION"),
        (Section::Console, "https://megapc.tn/shop/CONSOLES/NINTENDO"),
        (Section::Controller, "https://megapc.tn/shop/ACCESSOIRES/MANETTE"),
        (Section::ConsoleAccessories, "https://megapc.tn/shop/CONSOLES/ACCESSOIRES%20CONSOLES"),
        (Section::ConsoleAccessories, "https://megapc.tn/shop/ACCESSOIRES/VOLANT"),
        (Section::Smartphone, "https://megapc.tn/shop/SMARTPHONE/IPHONE"),
        (Section::Smartphone, "https://megapc.tn/shop/SMARTPHONE/ANDROID"),
        (Section::Tablet, "https://megapc.tn/shop/TABLETTE/ANDROID%20TABLETTE"),
        (Section::Smartwatch, "https://megapc.tn/shop/SMARTPHONE%20ACCESSOIRES/SMARTWATCH"),
        (Section::Television, "https://megapc.tn/shop/IMAGE%20&%20SON/SMART%20TV"),
    ],
};

pub struct MegaPC;

impl Retailer for MegaPC {
    fn config(&self) -> &RetailerConfig {
        &CONFIG
    }

    fn parse_basics(&self, element: ElementRef) -> Result<(String, String, String), ProductParseError> {
        let title = element.select_attr("title", "title")?;

        let url = element.select_elem(&CONFIG.title_sel, "url")?.select_attr("href", "url")?;
        let url = format!("https://megapc.tn{url}");
        validate_url(&url)?;

        let image = element.select_elem(&CONFIG.image_sel, "image")?.select_attr("src", "image url")?;
        let image = format!("https://megapc.tn{image}");
        validate_url(&image)?;

        Ok((title, url, image))
    }
}
